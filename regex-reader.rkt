...
(define (parse-/ in src line col pos)
  ...
  (datum->syntax #f
                 (pregexp (escape-backslash regex-expr))
...
;;; wrap-backslash: helper function to escape backslash if "\\<char>" can be recognized by racket
;;; reader
;;;
;;; "\\d\\t" -> "\"\\\\d\\t\""
;;;
...
;;;
;;; escape-backslash will parse the escapped character in a raw string in the same way the racket
...
(define (escape-backslash str)
  (read (open-input-string (wrap-backslash str))))
